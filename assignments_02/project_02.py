...
print('shape before removing Zeros: ', df.shape) #(395, 18)
...
for name, value in sorted_corrs:
...
# model: use failures alone to predict G3. Split into training and test sets 
# (80/20, random_state=42), fit a LinearRegression model, and print the slope, RMSE, 
# and R² on the test set.
# creating model  => y= ax + b => g3=y , X= failure => G3=a(failures)+b
...
# comment 
# each extra past failure lowers the predicted final grade by about 1.43 points, and the model misses by about 3 points on a 0-20 scale.
# R² is low (0.09), so the model is worse than expected and failures alone do not predict grades very well.
#----------------------------------------------------------------------------------
...
plt.savefig("outputs/predicted_vs_actual2.png")
plt.close()
...
# The Test R² increased to 0.75, so adding G1 makes the model much better at predicting final grades.
# A high R² does not mean G1 causes G3; it is useful for finding students who may struggle after first 
# term, but earlier support would need data before G1.